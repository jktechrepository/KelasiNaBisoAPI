...
# ============================================================================
# Script de Test : Bulk Insert depuis Excel
# ============================================================================
# Ce script teste l'upload et le traitement de fichiers Excel pour les inscriptions
# ============================================================================
...
NOM_ECOLE="${NOM_ECOLE:-Ecole Test}"

# Variables pour l'authentification
EMAIL="${EMAIL:-admin@kelasinabiso.cd}"
PASSWORD="${PASSWORD:-12345678}"

echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  TEST : Bulk Insert depuis Excel${NC}"
echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}"
echo ""
...
if [ "$HAS_JQ" = true ]; then
    ...
    USER_ROLE=$(echo "$AUTH_RESPONSE" | jq -r '.role // empty')
else
...
TEMPLATE_FILE="template_inscriptions.xlsx"

curl -k -s -X GET "${API_URL}/Inscription/template-excel" \
...
# Note : Pour créer un vrai fichier Excel, on aurait besoin de Python avec openpyxl
# Pour ce test, on va utiliser le template téléchargé et le modifier si possible
# Sinon, on va créer un fichier de test minimal

TEST_FILE="test_inscriptions.xlsx"
...
UPLOAD_RESPONSE=$(curl -k -s -X POST "${API_URL}/Inscription/bulk-excel/${NOM_ECOLE}" \
    -H "Authorization: Bearer ${TOKEN}" \
    -F "file=@${TEST_FILE}")
...
echo "  2. Remplissez avec des données valides (IdEcole, IdClasse, IdAnneeScolaire existants)"
