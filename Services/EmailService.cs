...
{
    /// <summary>
    /// Service d'envoi d'emails via SMTP (mail.kelasinabiso.com)
    /// </summary>
    public class EmailService : IEmailService
    {
        ...
        public EmailService(IConfiguration configuration, ILogger<EmailService> logger)
        {
            ...
            _smtpServer = _configuration["EmailSettings:SmtpServer"] ?? "mail.kelasinabiso.com";
            _smtpPort = int.Parse(_configuration["EmailSettings:Port"] ?? "465");
            _enableSsl = bool.Parse(_configuration["EmailSettings:EnableSsl"] ?? "true");
            _senderEmail = _configuration["EmailSettings:SenderEmail"] ?? "";
            _senderPassword = _configuration["EmailSettings:Password"] ?? "";
            _senderName = _configuration["EmailSettings:SenderName"] ?? "KelasiNaBiso";
        }
            ...
            string matriculeEnfant = null)
        {
            try
            {
                ...
                // Créer le sujet
                string subject = $"Bienvenue sur KelasiNaBiso - Vos identifiants de connexion";
        ...
        public async Task<bool> SendPasswordResetEmailAsync(string email, string nomComplet, string resetToken)
        {
            try
            {
                ...
                string subject = "KelasiNaBiso - Réinitialisation de votre mot de passe";
        ...
        public async Task<bool> SendPasswordChangedConfirmationEmailAsync(string email, string nomComplet, DateTime dateChangement, string adresseIP = null)
        {
            try
            {
                ...
                string subject = "✅ Confirmation de changement de mot de passe - KelasiNaBiso";
            ...
            string? replyToName = null)
        {
                    ...
                    using (var mailMessage = new MailMessage())
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(replyToEmail))
                        {
                            mailMessage.ReplyToList.Add(new MailAddress(
                                replyToEmail.Trim(),
                                string.IsNullOrWhiteSpace(replyToName) ? replyToEmail.Trim() : replyToName.Trim()));
                        }
            ...
            {
                _logger.LogError(smtpEx, $"❌ Erreur SMTP lors de l'envoi à {toEmail}: {smtpEx.Message}");
                return false;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"❌ Erreur générale lors de l'envoi à {toEmail}: {ex.Message}");
                return false;
            }
        }
...
<head>
    ...
    <title>Bienvenue sur KelasiNaBiso</title>
    <style>
        body {{
...
<body>
    <div class='email-wrapper'>
        <div class='header'>
            <h1 class='header-logo'>Kelasi<span class='highlight'>Na</span>Biso</h1>
        </div>
        
        <div class='content'>
            <h2 class='title'>Bienvenue sur KelasiNaBiso</h2>
            ...
            <p class='message'>
                ...
                Vous faites maintenant partie de <strong>{nomEcole}</strong> en tant que <span class='role-badge'>{(!string.IsNullOrWhiteSpace(fonction) ? fonction : role)}</span>.
            </p>
            ...
            <div class='highlight-box'>
                <p class='highlight-box-title'>En tant que parent, vous pourrez :</p>
                <ul style='margin: 10px 0; padding-left: 25px; color: #666666;'>
                    <li style='margin: 5px 0;'>Suivre la scolarité de votre enfant en temps réel</li>
                    ...
                    <li style='margin: 5px 0;'>Communiquer avec les enseignants</li>
            ...
            <div style='text-align: center; margin: 30px 0;'>
                <a href='https://www.kelasinabiso.com/' class='button'>Se connecter maintenant</a>
            </div>
        ...
        <div class='footer'>
            <p class='footer-text'>Cet email a été envoyé automatiquement par KelasiNaBiso Platform.</p>
            <p class='footer-text'>© {DateTime.Now.Year} KelasiNaBiso. Tous droits réservés.</p>
        </div>
    </div>
</body>
</html>";
...
🔗 Se connecter : https://www.kelasinabiso.com/
...
KelasiNaBiso Platform - Votre partenaire éducatif
© {DateTime.Now.Year} KelasiNaBiso. Tous droits réservés.
";
        }
...
<head>
    <meta charset='UTF-8'>
    <meta name='viewport' content='width=device-width, initial-scale=1.0'>
    <title>Vérifiez votre identité - KelasiNaBiso</title>
    <style>
        body {{
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
            background-color: #f5f5f5;
...
<body>
    <div class='email-wrapper'>
        ...
        <div class='content'>
            ...
            <p class='message'>
                Nous avons identifié une demande de réinitialisation de mot de passe pour l'utilisateur <strong>{nomComplet}</strong> sur votre compte KelasiNaBiso. 
...
<body>
    <div class='container'>
        ...
        <div class='footer'>
            <p><strong>KelasiNaBiso Platform</strong></p>
            <p style='font-size: 12px; color: #999;'>© {DateTime.Now.Year} KelasiNaBiso. Tous droits réservés.</p>
        ...
        /// </summary>
        public async Task<bool> SendGenericEmailAsync(
            string toEmail,
            string toName,
            string subject,
            string plainTextBody,
            ...
            string? replyToName = null)
        {
            return await SendEmailAsync(toEmail, toName, subject, plainTextBody, htmlBody, replyToEmail, replyToName);
        }
    }
}
