...
namespace KelasiNaBiso.Services.Notifications
{
    public class NotificationSender : INotificationSender
    {
        ...
        public async Task SendAsync(NotificationDispatchResult dispatchResult, CancellationToken cancellationToken = default)
        {
            ...
            var utilisateur = context.UtilisateurTuteur;
            ...
            {
                channelStatus["Push"] = DescribeDisabled("push", context, utilisateur, tuteur);
            }
            ...
            {
                channelStatus["InApp"] = DescribeDisabled("in-app", context, utilisateur, tuteur);
            }

            // SMS ----------------------------------------------------------------------
            if (message.Sms?.IsEnabled == true && tuteur != null && !string.IsNullOrWhiteSpace(tuteur.Telephone))
            {
                try
                {
                    ...
                    {
                        _logger.LogInformation("✅ SMS {Kind} envoyé au tuteur {TuteurId}", kind, tuteur.IdTuteur);
                        channelStatus["Sms"] = "envoyé";
                    }
                    else if (smsLog != null && smsLog.Statut == "failed")
                    {
                        _logger.LogWarning("⚠️ SMS {Kind} échoué pour tuteur {TuteurId}: {Erreur}", kind, tuteur.IdTuteur, smsLog.MessageErreur);
                        channelStatus["Sms"] = "échec";
                    }
                    else
                    {
                        _logger.LogWarning("⚠️ SMS {Kind} non envoyé (service désactivé ou numéro invalide) pour tuteur {TuteurId}", kind, tuteur.IdTuteur);
                        channelStatus["Sms"] = "non envoyé";
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "❌ Erreur SMS {Kind} pour tuteur {TuteurId}", kind, tuteur.IdTuteur);
                    channelStatus["Sms"] = "erreur";
                }
            }
            else
            {
                channelStatus["Sms"] = DescribeDisabled("sms", context, utilisateur, tuteur);
            }

            _logger.LogInformation("📡 Notification {Kind} -> Dest:{Dest} | Push:{Push} | InApp:{InApp} | Sms:{Sms}",
                kind,
                utilisateur?.IdUtilisateur ?? tuteur?.IdTuteur,
                channelStatus["Push"],
                channelStatus["InApp"],
                channelStatus["Sms"]);
        }

        private string DescribeDisabled(string channel, NotificationContext context, Utilisateur? utilisateur, Tuteur? tuteur)
        {
            return channel.ToLowerInvariant() switch
            {
                ...
                "in-app" => utilisateur == null
                    ...
                    : context.Preferences?.TryGetValue("inapp", out var optInApp) == true && !optInApp
                        ...
                        : "ignoré (désactivé)",
                "sms" =>
                    context.AcceptsSms == false ? "ignoré (école refuse SMS)" :
                    context.Preferences?.TryGetValue("sms", out var optSms) == true && !optSms ? "ignoré (opt-out)" :
                    tuteur == null || string.IsNullOrWhiteSpace(tuteur.Telephone) ? "ignoré (aucun numéro)" :
                    "ignoré (désactivé)",
                _ => "ignoré"
            };
        }
        ...
        private async Task<int?> PersistInAppNotificationAsync(NotificationContext context, InAppNotificationMessage inApp, int idDestinataire, CancellationToken cancellationToken)
        {
            try
            {
                var notification = new Notification
                {
                    ...
                    IdEcole = context.Ecole?.IdEcole,
                    IdEleve = context.Eleve?.IdEleve,
                    IdAgent = null,
                    CanalUtilise = "InApp",
                    Priorite = "INFO",
