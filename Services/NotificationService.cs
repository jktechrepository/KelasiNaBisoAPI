...
namespace KelasiNaBiso.Services
{
    ...
    /// </summary>
    public class NotificationService : INotificationService, INotificationRepository
    {
            ...
            string priorite = "NORMALE")
        {
            try
            {
                ...
                {
                    _logger.LogWarning($"Aucun destinataire trouvé pour réunion (École: {idEcole}, Classe: {idClasse})");
                    return 0;
                }
                ...
                if (idEcole.HasValue) donnees.Add("idEcole", idEcole.Value.ToString());
            ...
            int? idClasse = null)
        {
            try
            {
                ...
                {
                    // Tous les parents du système
                    idsUtilisateurs = await _context.Utilisateurs
                        .Where(u => u.IdTuteur != null && u.Statut == true)
                        .Select(u => u.IdUtilisateur)
                        .Distinct()
                        .ToListAsync();
                }
        ...
        {
            return await _context.Notifications
                .Where(n => n.IdDestinataire == idDestinataire)
                .OrderByDescending(n => n.DateCreation)
                .ToListAsync();
        }
        ...
        {
            return await _context.Notifications
                .Where(n => n.IdExpediteur == idExpediteur)
                .OrderByDescending(n => n.DateCreation)
                .ToListAsync();
        }

        public async Task<IEnumerable<Notification>> GetByEcoleAsync(int idEcole)
        {
            return await _context.Notifications
                .Where(n => n.IdEcole == idEcole)
                .OrderByDescending(n => n.DateCreation)
                .ToListAsync();
        }
        ...
        {
            return await _context.Notifications
                .Where(n => n.TypeNotification == type)
                .OrderByDescending(n => n.DateCreation)
                .ToListAsync();
        }
