...
namespace KelasiNaBiso.Services
{
    ...
    public class AuthorizationService : IUserAuthorizationService
    {
        ...
        public AuthorizationService(KelasiNaBisoDbContext context, ILogger<AuthorizationService> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        private readonly Dictionary<string, Dictionary<string, List<string>>> _permissions = new()
        {
            ["Super-Admin"] = new()
            {
                ["Ecole"] = new() { "Create", "Read", "Update", "Delete" },
            ...
            ["Admin"] = new()
            {
                ["Ecole"] = new() { "Read" }, // Son école uniquement
        ...
        public async Task<bool> IsInScopeAsync(int userId, int targetId, string scope)
        {
            try
            {
                ...
                return scope switch
                {
                    "Ecole" => userScope.EcoleId.HasValue && 
                               await IsEntityInEcole(targetId, userScope.EcoleId.Value),
        ...
        public async Task<UserScope> GetUserScopeAsync(int userId)
        {
            try
            {
                ...
                {
                    case "Enseignant":
                        // TODO: Ajouter une relation Utilisateur -> Enseignant
                        // Pour l'instant, on utilise une logique temporaire
                        break;

                    case "Tuteur":
                        // Pour le tuteur, on récupère ses enfants
                        var enfants = await _context.Eleves
                            .Where(e => e.IdTuteur == user.IdUtilisateur)
                            .Select(e => e.IdEleve)
                            .ToListAsync();
                        scope.EleveIds = enfants;
                        break;
                ...
                _logger.LogInformation("Scope utilisateur {UserId}: {Role}, École: {EcoleId}, Classe: {ClasseId}, Élèves: {EleveCount}",
                    userId, scope.RoleName, scope.EcoleId, scope.ClasseId, scope.EleveIds.Count);
        ...
        private async Task<bool> IsEntityInEcole(int entityId, int ecoleId)
        {
