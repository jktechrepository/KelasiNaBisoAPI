...
namespace KelasiNaBisoAPI.Services
{
    ...
    public class FirebaseNotificationService : IFirebaseNotificationService
    {
        ...
        /// </summary>
        public async Task<int> EnvoyerNotificationParEcoleAsync(
            int idEcole,
            string titre,
            string corps,
            Dictionary<string, string>? donnees = null)
        {
            try
            {
                ...
                if (!tokens.Any())
                {
                    _logger.LogWarning($"Aucun token FCM actif trouvé pour l'école {idEcole}");
                    return 0;
                }
                ...
                _logger.LogInformation($"Notification envoyée à l'école {idEcole}. Succès: {response.SuccessCount}/{tokens.Count()}");
