    ...
    public class AuditController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 50)
        {
            try
            {
                ...
                if (currentUserRole != "Super-Admin" && currentUserSchoolId != idEcole)
                {
                    return Forbid();
                }

                var activities = await _auditService.GetSchoolActivitiesAsync(idEcole, from, to, page, pageSize);
            ...
            {
                _logger.LogError(ex, $"Erreur lors de la récupération des activités de l'école {idEcole}");
                return StatusCode(500, new { message = "Erreur lors de la récupération des activités" });
            }
        }
            ...
            [FromQuery] int? idEcole = null)
        {
            try
            {
                ...
                var stats = await _auditService.GetStatisticsAsync(from, to, idEcole);
