...
namespace KelasiNaBisoAPI.Controllers
{
    ...
    public class NotificationPushController : ControllerBase
    {
        ...
        /// </summary>
        [HttpPost("ecole/{idEcole}")]
        public async Task<ActionResult> EnvoyerParEcole(
            int idEcole,
            [FromBody] NotificationPushRequest request)
        {
            try
            {
                ...
                _logger.LogInformation($"Envoi de notification push à l'école {idEcole}");

                var count = await _firebaseService.EnvoyerNotificationParEcoleAsync(
                    idEcole, request.Titre, request.Corps, request.Donnees);
            ...
            {
                _logger.LogError(ex, $"Erreur lors de l'envoi de notification à l'école {idEcole}");
                return StatusCode(500, new { message = "Erreur interne du serveur", error = ex.Message });
            }
        }
