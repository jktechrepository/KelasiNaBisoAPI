...
SELECT DISTINCT
    ...
    -- Élève
    e.IdEleve,
    e.ReferenceEleve, -- Maintenant de type GUID
    e.Prenom,
    e.Nom,
    e.Postnom,
    ...
    CASE WHEN e.Statut = 1 THEN 'True' ELSE 'False' END AS StatutEleve,
    ...
    CASE WHEN t.Statut = 1 THEN 'True' ELSE 'False' END AS StatutTuteur,
    t.PhotoTuteurUrl,
...
PRINT 'Vue VuePointagePresenceParEcole recréée avec succès!';
